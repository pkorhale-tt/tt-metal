...
// Fixes vs original:
// 1. Removed stray copy_tile_to_dst_init_short(cb_data1_r) at top of loop.
//    Each maths_sfpu_* helper issues its own UNPACK init internally.
// 2. Output push order changed to out0_r, out0_i, out1_r, out1_i so it
//    matches the writer's cb_wait_front order — prevents CB-fill deadlock.
// 3. cb_pop_front for inputs moved to after tile_regs_commit but before
//    tile_regs_wait (paper Listing 1.3 order) for better pipeline overlap.
...
void kernel_main() {
    ...
    for (uint32_t step = 0; step < num_steps; ++step) {
        for (uint32_t chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            // Push out0 BEFORE out1 so writer's cb_wait_front(cb_out0_r) is
            // satisfied immediately — prevents CB-fill deadlock at depth 2.
            maths_sfpu_add(cb_data0_r, cb_f0, cb_out0_r);   // out0_r = data0_r + f0
            maths_sfpu_add(cb_data0_i, cb_f1, cb_out0_i);   // out0_i = data0_i + f1
            maths_sfpu_sub(cb_data0_r, cb_f0, cb_out1_r);   // out1_r = data0_r - f0
            maths_sfpu_sub(cb_data0_i, cb_f1, cb_out1_i);   // out1_i = data0_i - f1
