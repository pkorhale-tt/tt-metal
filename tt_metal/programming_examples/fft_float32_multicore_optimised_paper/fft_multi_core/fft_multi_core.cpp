...
// CB indices – must match all kernels exactly.
...
constexpr uint32_t CB_STEP_SYNC  = 24;  // writer -> reader per-step barrier token

// SRAM base: place data above CB region.
...
int main(int argc, char** argv) {
    try {
        ...
        if (nRow > 16384) {
            throw std::runtime_error("nRow > 16384 exceeds SRAM budget");
        }
...

        {
            ...
            const uint32_t chunkSize = halfN;
