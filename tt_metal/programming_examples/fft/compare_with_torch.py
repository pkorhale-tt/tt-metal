...
"""
compare_with_torch.py — run the tt-metal FFT on a random complex signal of
length N, run torch.fft.fft on the same signal, and report accuracy.
...
Usage:
    ...
    python compare_with_torch.py --N 4096 --bin /path/to/metal_example_fft_vs_torch

Environment variables:
    FFT_BIN   : path to the metal_example_fft_vs_torch binary (overridden by --bin).
                Default is "./build/programming_examples/fft/metal_example_fft_vs_torch".
...
The script:
  ...
  2. Loads the input signal and runs torch.fft.fft on it (complex128 for
     maximum reference accuracy, then compares in complex64 space).
  3. Loads the tt-metal output and computes:
         abs_err  = max_k |X_tt[k] - X_torch[k]|
         rel_err  = abs_err / max_k |X_torch[k]|
         mean_err = mean_k |X_tt[k] - X_torch[k]|
         snr_dB   = 10 * log10(||X_torch||^2 / ||X_tt - X_torch||^2)
  4. Prints the top-5 worst bins and a pass/fail verdict.
"""
...
DEFAULT_BIN = "./build/programming_examples/fft/metal_example_fft_vs_torch"
...
def run_tt_metal(binary: str, N: int, seed: int, in_path: str, out_path: str) -> None:
    if not os.path.exists(binary):
        sys.exit(f"Binary not found: {binary}\n"
                 f"Build it with:\n"
                 f"    cmake --build build --target metal_example_fft_vs_torch -j\n"
...
def main() -> None:
    ...
    ap.add_argument("--bin", dest="binary",
                    ...
                    help="path to metal_example_fft_vs_torch")
    ap.add_argument("--tol", type=float, default=2e-3,
                    help="relative-error pass threshold (default 2e-3)")
    ...
    tmpdir = tempfile.mkdtemp(prefix="fft_vs_torch_")
    ...
    try:
        ...
        # ── Load results ────────────────────────────────────────────────
        x     = load_complex_file(in_path)        # input signal
        X_tt  = load_complex_file(out_path)       # tt-metal output
        if x.shape != (args.N,) or X_tt.shape != (args.N,):
            sys.exit(f"shape mismatch: x={x.shape}, X_tt={X_tt.shape}, expected ({args.N},)")

        # ── Torch reference (complex128 for a clean reference) ──────────
        ...
        print()
        print("=" * 72)
        print(f"  tt-metal FFT vs torch.fft.fft  (N={args.N}, seed={args.seed})")
        print("=" * 72)
        ...
        print()
        print(f"  [{status}]  tt-metal FFT matches torch.fft.fft within tolerance "
              f"{args.tol:g}" if pass_ else
              f"  [{status}]  tt-metal FFT DOES NOT match torch.fft.fft within "
