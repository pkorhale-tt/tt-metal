...
//
// fft_vs_torch.cpp — runs the tt-metal FFT on a user-specified N and writes
// the input signal and the FFT output to plain-text files that a Python
// companion script (compare_with_torch.py) reads and compares against
// torch.fft.fft.
//
// Usage:
//     metal_example_fft_vs_torch <N> [seed] [in_path] [out_path]
//
//   N         : FFT length, power of two, 2..65536.
...
int main(int argc, char** argv) {
    ...
    std::printf("[fft_vs_torch] wrote %s  and  %s\n", in_path.c_str(), out_path.c_str());
    std::printf("[fft_vs_torch] N=%u  seed=%u  wall=%.1f ms (incl. JIT on cold run)\n",
                N, seed, ms);
