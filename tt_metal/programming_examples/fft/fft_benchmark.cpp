...
//
// fft_benchmark.cpp — demonstrates the impact of the plan cache.
//
// Runs fft() repeatedly on the same N and prints per-iteration timings.
// The first iteration pays program build + twiddle upload (slow). Every
// subsequent iteration hits the cached plan and should be ~10x faster.
//
// Usage:
//   ./build/programming_examples/fft/metal_example_fft_benchmark [N] [iters]
//   e.g.   ./.../metal_example_fft_benchmark 65536 100
...
int main(int argc, char** argv) {
    ...
    std::printf("\n=== FFT benchmark: N=%u, iterations=%u ===\n\n", N, iters);

    auto md = tt::tt_metal::distributed::MeshDevice::create_unit_mesh(0);

    // Prepare a deterministic random complex signal.
    ...
    for (uint32_t i = 0; i < iters; ++i) {
        ...
        auto out = fft_example::fft(md, signal);
        ...
        // Print the first few + last few so output stays readable for big iters.
        if (i < 5 || i >= iters - 3) {
    ...
    // Summary stats (skip first iteration since it's the build cost).
