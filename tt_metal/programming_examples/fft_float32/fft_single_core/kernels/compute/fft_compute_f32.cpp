...
inline void math_op(uint32_t in0_cb, uint32_t in1_cb, uint32_t out_cb) {
    ...
    if constexpr (OP == SUB) sub_tiles_init(in0_cb, in1_cb);
    if constexpr (OP == MUL) mul_tiles_init(in0_cb, in1_cb);

    tile_regs_acquire();

    if constexpr (OP == ADD) add_tiles(in0_cb, in1_cb, 0, 0, 0);
    if constexpr (OP == SUB) sub_tiles(in0_cb, in1_cb, 0, 0, 0);
    if constexpr (OP == MUL) mul_tiles(in0_cb, in1_cb, 0, 0, 0);
