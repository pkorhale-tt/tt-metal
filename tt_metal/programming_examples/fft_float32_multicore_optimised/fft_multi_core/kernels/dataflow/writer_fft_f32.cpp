        ...
        for (uint32_t stage = 0; stage < num_stages; stage++) {
            ...
            if (is_last) {
                ...
                for (uint32_t t = 0; t < num_tiles; t++) {
                    noc_async_write_tile(row_tile_base + t, out0_r_gen, src0r + t * tile_bytes);
                    noc_async_write_tile(row_tile_base + t, out0_i_gen, src0i + t * tile_bytes);
                    noc_async_write_tile(row_tile_base + t, out1_r_gen, src1r + t * tile_bytes);
                    noc_async_write_tile(row_tile_base + t, out1_i_gen, src1i + t * tile_bytes);
                }
                ...
                // Clear full destination tiles so compute never reads stale L1 values.
                for (uint32_t lp = 0; lp < num_tiles * (tile_bytes / ELEM); ++lp) {
                        ...
                        for (uint32_t j2 = 0; j2 < half_m2; j2++) {
                            {
                                ...
                                wr32(dst_er + dst * ELEM, rd32(srcr + idx * ELEM));
                                wr32(dst_ei + dst * ELEM, rd32(srci + idx * ELEM));
                            }
                            {
                                ...
                                wr32(dst_or + dst * ELEM, rd32(srcr + idx * ELEM));
                                wr32(dst_oi + dst * ELEM, rd32(srci + idx * ELEM));
                            }
                            dst++;
                        }
                    }
