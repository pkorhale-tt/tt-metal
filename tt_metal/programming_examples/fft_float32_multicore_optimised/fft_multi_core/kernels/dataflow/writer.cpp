// writer_debug.cpp - minimal: drain compute outputs, write last stage to DRAM
#include <cstdint>
...
void kernel_main() {
    ...
    const InterleavedAddrGenFast<true> gen_er = { .bank_base_address = out_er_addr, .page_size = tile_bytes, .data_format = fmt };
    const InterleavedAddrGenFast<true> gen_ei = { .bank_base_address = out_ei_addr, .page_size = tile_bytes, .data_format = fmt };
    const InterleavedAddrGenFast<true> gen_or = { .bank_base_address = out_or_addr, .page_size = tile_bytes, .data_format = fmt };
    const InterleavedAddrGenFast<true> gen_oi = { .bank_base_address = out_oi_addr, .page_size = tile_bytes, .data_format = fmt };
        ...
        for (uint32_t stage = 0; stage < num_stages; ++stage) {
            ...
            if (is_last) {
                ...
                for (uint32_t t = 0; t < tiles_per_row; ++t) {
                    noc_async_write_tile(row_tile_base + t, gen_er, src_er + t * tile_bytes);
                    noc_async_write_tile(row_tile_base + t, gen_ei, src_ei + t * tile_bytes);
                    noc_async_write_tile(row_tile_base + t, gen_or, src_or + t * tile_bytes);
                    noc_async_write_tile(row_tile_base + t, gen_oi, src_oi + t * tile_bytes);
                }
                ...
                // Intermediate: shuffle into L1 scratch, pop outputs, push feedback
                ...
                if (G2 > 0) {
                    ...
                    for (uint32_t g2 = 0; g2 < G2; ++g2) {
                        ...
                        for (uint32_t j2 = 0; j2 < half_m2; ++j2, ++dst) {
                            {
                                const uint32_t f = base_e + j2;
                                const uint32_t g_cur = f >> log2m;
                                const uint32_t offset = f & m_mask;
                                uint32_t idx, sr, si;
                                if (offset < half_m) { idx = g_cur * half_m + offset; sr = src_er; si = src_ei; }
                                else { idx = g_cur * half_m + (offset - half_m); sr = src_or; si = src_oi; }
                                wr32(scr_er + dst * F, rd32(sr + idx * F));
                                wr32(scr_ei + dst * F, rd32(si + idx * F));
                            }
                            {
                                ...
                                wr32(scr_or + dst * F, rd32(sr + idx * F));
                                wr32(scr_oi + dst * F, rd32(si + idx * F));
                            }
                        }
                    }
                }

                // KEY: pop BEFORE reserve to prevent deadlock
                cb_pop_front(CB_OER, tiles_per_row);
                cb_pop_front(CB_OEI, tiles_per_row);
                cb_pop_front(CB_OOR, tiles_per_row);
                cb_pop_front(CB_OOI, tiles_per_row);
