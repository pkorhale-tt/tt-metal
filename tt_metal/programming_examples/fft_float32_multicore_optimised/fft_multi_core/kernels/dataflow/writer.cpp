...
void kernel_main() {
    ...
    // Total float elements per row in tile storage and actual valid FFT lanes.
    ...
    for (uint32_t row = 0; row < rows_per_core; ++row) {
        ...
        for (uint32_t stage = 0; stage < num_stages; ++stage) {
            ...
            if (is_last) {
                ...
                // Only the first `half_N` entries are valid FFT lanes. Using
                all padded tile lanes here causes zeros to get shuffled as if
                they were real data.
                const uint32_t G2 = (half_m2 <= valid_elems) ? (valid_elems / half_m2) : 1u;
                ...
                for (uint32_t g2 = 0; g2 < G2; ++g2) {
                    ...
                    for (uint32_t j2 = 0; j2 < half_m2 && dst_idx < valid_elems; ++j2, ++dst_idx) {
