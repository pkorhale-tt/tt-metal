...
void kernel_main() {
    ...
    // Compact twiddles are stored in DRAM as one full tile page because they
    // are fetched with noc_async_read_tile(). Only the first half_N floats
    // are valid; the tail is zero-padded by the host.
