...
def rand(N: int, complex: bool = True, seed: Optional[int] = None) -> np.ndarray:
    ...
    if _HAVE_TTNN:
        try:
            ...
            if complex:
                # ttnn.rand is uniform [0,1); recenter to [-1, 1)
                arr = arr * 2.0 - 1.0
                return (arr[:, 0] + 1j * arr[:, 1]).astype(np.complex64)
            return (arr * 2.0 - 1.0).astype(np.float32)
        except Exception as e:
            # Fall through to numpy if the device isn't usable
            print(f"[tt_fft.rand] ttnn unavailable ({e}); using numpy")
...
def randn(N: int, complex: bool = True, seed: Optional[int] = None) -> np.ndarray:
    """Standard-normal random complex signal. Tries ttnn first, falls back to numpy."""
    if _HAVE_TTNN:
        try:
            dev = _get_ttnn_device()
            shape = (N, 2) if complex else (N,)
            # ttnn.randn may not exist on every build; use rand + Box-Muller fallback
            if hasattr(ttnn, "randn"):
                t = ttnn.randn(shape, dtype=ttnn.bfloat16,
                               layout=ttnn.ROW_MAJOR_LAYOUT, device=dev)
                arr = from_ttnn(t).astype(np.float32)
            else:
            ...
            print(f"[tt_fft.randn] ttnn unavailable ({e}); using numpy")
    rng = np.random.default_rng(seed)
    if complex:
