...
//
// fft_reader.cpp (bf16) — BRISC0 / reader (multi-core, DMA-optimised)
//
// Identical structure to the fp32 version, except elements are 2 bytes
// (bf16) instead of 4 bytes. The reader never interprets the values, it
// only moves bytes between L1 buffers, so we use uint16_t* for the small
// scalar fallback and 2-byte strides everywhere.
...
// Below this many bytes the NoC setup cost outweighs the DMA win, so we
// drop to scalar copies. With bf16 (2 B/element) 64 B == 32 elements.
...
void kernel_main() {
...
#if FFT_DPRINT_HELLO
    DPRINT << "fft_bf16: my_core=" << my_core << " alive\n";
#endif
    ...
    volatile tt_l1_ptr uint16_t* const state_r =
        reinterpret_cast<volatile tt_l1_ptr uint16_t*>(state_r_l1);
    volatile tt_l1_ptr uint16_t* const state_i =
        reinterpret_cast<volatile tt_l1_ptr uint16_t*>(state_i_l1);
    ...
    for (uint32_t s = 0; s < LOG2N_LOCAL; ++s) {
        ...
        const uint32_t block_bytes  = stride * 2u;            // bf16: 2 B / elem
        ...
        if (use_dma) {
            for (uint32_t g = 0; g < num_groups; ++g) {
                const uint32_t src_even = state_r_l1 + (g * group_size)           * 2u;
                const uint32_t src_odd  = state_r_l1 + (g * group_size + stride)  * 2u;
                const uint32_t src_evi  = state_i_l1 + (g * group_size)           * 2u;
                const uint32_t src_odi  = state_i_l1 + (g * group_size + stride)  * 2u;
                const uint32_t dst_even = even_r_l1  + (g * stride)               * 2u;
                const uint32_t dst_odd  = odd_r_l1   + (g * stride)               * 2u;
                const uint32_t dst_evi  = even_i_l1  + (g * stride)               * 2u;
                const uint32_t dst_odi  = odd_i_l1   + (g * stride)               * 2u;
            ...
            volatile tt_l1_ptr uint16_t* const even_r =
                reinterpret_cast<volatile tt_l1_ptr uint16_t*>(even_r_l1);
            volatile tt_l1_ptr uint16_t* const even_i =
                reinterpret_cast<volatile tt_l1_ptr uint16_t*>(even_i_l1);
            volatile tt_l1_ptr uint16_t* const odd_r =
                reinterpret_cast<volatile tt_l1_ptr uint16_t*>(odd_r_l1);
            volatile tt_l1_ptr uint16_t* const odd_i =
                reinterpret_cast<volatile tt_l1_ptr uint16_t*>(odd_i_l1);
        ...
        if (use_dma) {
            for (uint32_t g = 0; g < num_groups; ++g) {
                const uint32_t dst_lo_r = state_r_l1 + (g * group_size)          * 2u;
                const uint32_t dst_hi_r = state_r_l1 + (g * group_size + stride) * 2u;
                const uint32_t dst_lo_i = state_i_l1 + (g * group_size)          * 2u;
                const uint32_t dst_hi_i = state_i_l1 + (g * group_size + stride) * 2u;
                const uint32_t src_o0r  = o0r_l1 + (g * stride) * 2u;
                const uint32_t src_o0i  = o0i_l1 + (g * stride) * 2u;
                const uint32_t src_o1r  = o1r_l1 + (g * stride) * 2u;
                const uint32_t src_o1i  = o1i_l1 + (g * stride) * 2u;
            ...
            volatile tt_l1_ptr uint16_t* const o0r =
                reinterpret_cast<volatile tt_l1_ptr uint16_t*>(o0r_l1);
            volatile tt_l1_ptr uint16_t* const o0i =
                reinterpret_cast<volatile tt_l1_ptr uint16_t*>(o0i_l1);
            volatile tt_l1_ptr uint16_t* const o1r =
                reinterpret_cast<volatile tt_l1_ptr uint16_t*>(o1r_l1);
            volatile tt_l1_ptr uint16_t* const o1i =
                reinterpret_cast<volatile tt_l1_ptr uint16_t*>(o1i_l1);
            for (uint32_t i = 0; i < LOCAL_PAIRS; ++i) {
                const uint32_t group = i >> s;
                const uint32_t pos   = i & mask;
                const uint32_t lo    = group * group_size + pos;
    ...
    if constexpr (P > 1) {
        {
            ...
            noc_async_write(state_r_l1, p0_rr, TILE_SIZE_BF16);
            noc_async_write(state_i_l1, p0_ri, TILE_SIZE_BF16);
            noc_async_write_barrier();
        ...
        for (uint32_t k = 0; k < LOG2P; ++k) {
            ...
            if (is_c_even) {
                async_local_memcpy(state_r_l1, even_r_l1, TILE_SIZE_BF16, my_noc_x, my_noc_y);
                async_local_memcpy(state_i_l1, even_i_l1, TILE_SIZE_BF16, my_noc_x, my_noc_y);
                async_local_memcpy(recv_r_l1,  odd_r_l1,  TILE_SIZE_BF16, my_noc_x, my_noc_y);
                async_local_memcpy(recv_i_l1,  odd_i_l1,  TILE_SIZE_BF16, my_noc_x, my_noc_y);
            } else {
                async_local_memcpy(recv_r_l1,  even_r_l1, TILE_SIZE_BF16, my_noc_x, my_noc_y);
                async_local_memcpy(recv_i_l1,  even_i_l1, TILE_SIZE_BF16, my_noc_x, my_noc_y);
                async_local_memcpy(state_r_l1, odd_r_l1,  TILE_SIZE_BF16, my_noc_x, my_noc_y);
                async_local_memcpy(state_i_l1, odd_i_l1,  TILE_SIZE_BF16, my_noc_x, my_noc_y);
            }
            noc_async_write_barrier();
            ...
            async_local_memcpy(src_r, state_r_l1, TILE_SIZE_BF16, my_noc_x, my_noc_y);
            async_local_memcpy(src_i, state_i_l1, TILE_SIZE_BF16, my_noc_x, my_noc_y);
            ...
            if (k + 1 < LOG2P) {
                ...
                noc_async_write(src_r, np_rr, TILE_SIZE_BF16);
                noc_async_write(src_i, np_ri, TILE_SIZE_BF16);
                noc_async_write_barrier();
