...
//
// fft_compute.cpp (bf16) — TRISC / compute
//
// Same butterfly logic as the fp32 version. CBs hold bf16 (2 B/element);
// the unpacker upcasts to fp32 in the DEST register (UnpackToDestFp32 set
// host-side), the SFPU does fp32 math, then pack back to bf16 in the CB.
// So per-stage math is fp32 but inter-stage storage is bf16 — that is the
// full bf16-vs-fp32 trade-off.
