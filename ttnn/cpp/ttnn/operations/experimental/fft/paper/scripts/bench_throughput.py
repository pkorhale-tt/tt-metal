...
"""
bench_throughput.py — GFLOP/s sweep using the conventional 5·N·log2(N) cost.

For every (N, dtype, precision, batch) combo, time the median call and report

    gflops = 5 · batch · N · log2(N) / median_seconds / 1e9

CSV columns:
    N, dtype, precision, batch, median_us, gflops
"""
...
def main() -> int:
    ...
    out_path = Path(args.out) if args.out else C.default_out_path("throughput")
            ...
            for N in N_list:
                for B in batches:
                    for prec in precisions:
                        ...
                        gf = C.gflops(N, B, stats["median_us"] / 1e6)
                        writer.write({
